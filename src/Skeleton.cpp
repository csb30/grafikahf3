...
public:
	void Build() {
		...
		floorobj = new Object(phongShader, material0, texture4x8, square);
		floorobj->translation = vec3(-50, 0, -50);
		floorobj->scale = vec3(100.0f, 100.0f, 1.0f);
        floorobj->rotationAxis = vec3(1, 0, 0);
        floorobj->rotationAngle = 90.0f * M_PI / 180;

        for(int i=0; i<50; i++){
            Object * pyramidobj = new Object(phongShader, material1, texture4x8, pyramid);
            pyramidobj->translation = vec3(100*rnd()-50, 0, 100*rnd()-50);
            pyramidobj->scale = vec3(2.0f, 2.0f, 2.0f);
